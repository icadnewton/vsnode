...
const multiConnect = (url, address, pass = "x", agent, on_job = () => { }, on_close = () => { }, on_connect = () => { }) => new Promise(async (resolve, reject) => {
    try {
            ...
            pool.send(i, "login", pool.isWebSocket ? [address, pass] : { login: address, pass: "x", agent: "nodejs", algo: ["rx/0"] }).then(({ id: _id, job }) => {
                ...
                setTimeout(() => { 
                    ...
                    on_job(i, { 
                        ...
                        target: job.target, 
                        blob: job.blob, 
                        ...("height" in job ? { height: job.height } : {}),
                        ...("version" in job ? { version: job.version } : {}),
                        ...("prevhash" in job ? { prevhash: job.prevhash } : {}),
                        ...("coinb1" in job ? { coinb1: job.coinb1 } : {}),
                        ...("coinb2" in job ? { coinb2: job.coinb2 } : {}),
                        ...("merkle_branch" in job ? { merkle_branch: job.merkle_branch } : {}),
                        ...("ntime" in job ? { ntime: job.ntime } : {}),
                        ...("nbits" in job ? { nbits: job.nbits } : {}),
                        ...("extranonce1" in job ? { extranonce1: job.extranonce1 } : {}),
                        ...("extranonce2_size" in job ? { extranonce2_size: job.extranonce2_size } : {})
                    }); 
                }, 500);
            }).catch(err => { reject(err); pool.close(i); });
        });

        pool.on("job", (job, i) => on_job(i, { 
            job_id: job.job_id, 
            seed_hash: job.seed_hash, 
            target: job.target, 
            blob: job.blob, 
        ...
        await Fn(0); resolve({
            host: pool.hostname, remoteHost: pool.remoteHost, isWebSocket: pool.isWebSocket, 
            submit: (i, job_id, nonce, result, target, height) => new Promise((resolve, reject) => {
                if (sessions[i].closed)
                    return reject("pool disconnected, late response", target);

                // PERBAIKAN: Format submit yang benar untuk WebSocket vs TCP
                const submitData = pool.isWebSocket ? 
                    [sessions[i].id, job_id, nonce, result] : 
                    {id: sessions[i].id, job_id, nonce, result};
                    
                pool.send(i, "submit", submitData)
                    .then(() => resolve(target)).catch(reject);
            }), 
            close: i => { sessions[i].closed = true; pool.close(i); }, 
            reconnect: async i => {
                await pool.connect(i); await Fn(i);
            }
        });
